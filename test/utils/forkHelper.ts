...
export async function setupFork(networkName: Networks, blockNumber?: number) {
  if (!blockNumber) {
    logger.log(`Setting up fork of ${networkName} at latest block.`, '🍴')
  } else {
    logger.log(`Setting up fork of ${networkName} at block ${blockNumber}`, '🍴')
  }
  ...
  logger.log(`Fork of ${networkName} is ready at blocknumber: ${currentBlock}`, '🍴')
}
