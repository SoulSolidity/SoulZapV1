...
export class SoulZap_UniV2 {
    ...
    async getZapData(dex: DEX, fromToken: string, amount: number | string, toToken: string, allowedPriceImpactPercentage: number, to: string): Promise<zapData | { error: string }> {
    ...
    async getZapDataNative(dex: DEX, amount: number | string, toToken: string, allowedPriceImpactPercentage: number, to: string): Promise<zapData | { error: string }> {
        const lensContract = this.getLensContract(dex);
        const zapData = await lensContract.getZapDataNative(amount, toToken, this.slippage * this.DENOMINATOR, to);
        const priceImpactError = await this.checkPriceImpact(zapData.priceImpactPercentage0, zapData.priceImpactPercentage1, allowedPriceImpactPercentage);
    ...
    protected checkPriceImpact(priceImpactPercentage0: number, priceImpactPercentage1: number, allowedPriceImpactPercentage: number): { error: string } | null {
