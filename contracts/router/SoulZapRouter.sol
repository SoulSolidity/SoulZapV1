...
import {ISoulZapRouter} from "./interfaces/ISoulZapRouter.sol";
import {ISoulTokenManager} from "./interfaces/ISoulTokenManager.sol";
import {IPermit2} from "./interfaces/IPermit2.sol";
import {SoulTokenManager} from "./SoulTokenManager.sol";
 ...
 */
contract SoulZapRouter is ISoulZapRouter, Ownable, Pausable, ReentrancyGuard {
    ...
    bytes32 private constant ORDER_TYPEHASH =
        keccak256(
            "Order(Input[] inputs,Output[] outputs,Relay relay,address user,address recipient)Input(address token,uint256 amount)Output(address token,uint256 minOutputAmount)Relay(address target,uint256 value,bytes data)"
        );
    /**
    ...
    constructor(address _permit2) {
        permit2 = _permit2;
        tokenManager = address(new SoulTokenManager());
    }
    ...
    function _propagateError(
        ...
        if (_returnedData.length == 0) revert ZapErrors.CallFailed(_target, _value, _data);
        assembly {
        ...
        for (uint256 i; i < outputsLength; ) {
            ...
            if (outputToken == address(0)) {
                ...
                if (balance < outputMinAmount) {
                    revert ZapErrors.Slippage(outputToken, outputMinAmount, balance);
                }
                if (balance > _relayValue) {
                    balance -= _relayValue;
                    (bool success, ) = _recipient.call{value: balance}("");
                    if (!success) revert ZapErrors.EtherTransferFailed(_recipient);
                }
            } else {
    ...
    function _executeRelay(Relay calldata _relay) private {
        ...
        if (relayTarget != address(0)) {
            if (relayTarget == permit2 || relayTarget == tokenManager) {
                revert ZapErrors.TargetingInvalidContract(relayTarget);
            }

            if (address(this).balance < relayValue) {
                revert ZapErrors.InsufficientRelayValue(address(this).balance, relayValue);
            }
    ...
    //     Input[] calldata _inputs
    // ) private view returns (IPermit2.SignatureTransferDetails[] memory) {
    ...
    //             to: address(this),
    //             requestedAmount: _inputs[i].amount
    //         });
